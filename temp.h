...
#ifndef LANEDETECTOR_LANEDETECTION_H
#define LANEDETECTOR_LANEDETECTION_H
...
//                cv::Point pt4(0, houghBin.rows);
                ...
                customLine(houghBin, pt3, pt4, houghBin.rows/3);
            }
            iterator++;
        }
    }
     ...
     * eventually turns out to be a binary image with hough lines.
     * */
    void getHoughPLines(cv::Mat &houghP, cv::Mat &houghPBin, cv::Mat &contours){
        linesP.clear();
        twoLines.clear();
        ...
        while (iterator!=linesP.end()) {
            ...
            if((thetaDeg > 25.0 || thetaDeg < -25.0) ){
                twoLines.push_back(*iterator);
                //std::cout << "theta2: " << thetaDeg << "\n";
                line(houghP, pt1, pt2, cv::Scalar(255), 5);
                customLine(houghPBin, pt3, pt4, houghPBin.rows/8, cv::Scalar(60));
            }
            iterator++;
        }
    }
    ...
    void genBinImg(cv::Mat &img){
        ...
        if(twoLines.size() > 1) {
            ...
            cv::fillPoly(img, ppt, npt, 1, cv::Scalar(0, 255, 0), 8);
        }
    }
    ...
    void customLine(cv::Mat &img, cv::Point a, cv::Point b,int distInY = 0, cv::Scalar color = cv::Scalar(255)){
        ...
        cv::Point p(0,img.rows - distInY), q(img.cols, img.rows);
...
#endif //LANEDETECTOR_LANEDETECTION_H
